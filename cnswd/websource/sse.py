...
class SSEPage(object):
    ...
    def _get_item_data(self, suffix, tab_css, info, date_cols):
        """获取股票列表信息"""
    ...
    def get_stock_list_a(self):
        ...
        tab_css = 'ul.nav:nth-child(1) > li:nth-child(1) > a:nth-child(1)'
        date_cols = ['A股上市日期']
        df = self._get_item_data(suffix, tab_css, '上市A股', date_cols)
    ...
    def get_stock_list_b(self):
        ...
        date_cols = ['B股上市日期']
        df = self._get_item_data(suffix, tab_css, '上市B股', date_cols)
        return df.drop('公告', axis=1)
